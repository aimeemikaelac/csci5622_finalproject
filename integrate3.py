...
POS_data = {}
POS = DictReader(open("questions_processed.csv"),'rb')
counter = 0
for ii in POS:
    # if counter==0:
    #     print ii
    if counter>0:
        POS_data[int(float(ii['r']))] = ii[None][2]
        # print ii[None][2]
    counter+=1
...
#     length2 = len(question_data[int(ii['question'])][2].split(' '))
#     # unigrams = ''
#     # # unigrams = []
#     # for jj in question_data[int(ii['question'])][2].split(' '):
#     #     jj = jj.strip(string.punctuation)
...
# rms = mean_squared_error(average, answers) ** 0.5
# print "RMS Accuracy Position Only = ", rms

############################################################################################
# Classifier for average question percent
...
#     else:
#         dev_train_feats.append(t)
#         dev_train_labels.append(float(ii['QuestPercent']))
...
# for ii in dev_test:
#     try:
#         answers.append(int(float(ii['QuestPercent'])))
#         average.append(one_q_per)
#     except KeyError:
#         print ii['id'],ii['QuestPercent']
...
with open('test.csv', 'rb') as test, open('test2.csv', 'wb') as test2:
    ...
    for row in csvreader:
        ...
        try:
            ...
            row['POS'] = POS_data[int(row['question'])]
        except KeyError:
            print int(row['id'])
